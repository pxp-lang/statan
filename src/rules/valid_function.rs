use pxp_parser::{node::Node, downcast::downcast, parser::ast::{FunctionCallExpression, Expression, identifiers::{Identifier, SimpleIdentifier}, arguments::{Argument, PositionalArgument, NamedArgument}}, lexer::byte_string::ByteString};
...
impl Rule for ValidFunctionRule {
    ...
    fn run(&mut self, node: &mut dyn Node, definitions: &DefinitionCollection, messages: &mut MessageCollector, context: &mut Context) {
        ...
        if definition.is_none() {
            ...
            messages.error(format!("Function `{}` (DBG: {}, {}) not found", function_name, context.resolve_name(function_name), {
        ...
        if function_call_expression.arguments.arguments.len() < min_arity {
            messages.error(format!("Function {}() requires {} arguments, {} given", function_name, min_arity, function_call_expression.arguments.arguments.len()), span.line);
            return;
        }

        if function_call_expression.arguments.arguments.len() > max_arity {
            messages.error(format!("Function {}() requires {} arguments, {} given", function_name, max_arity, function_call_expression.arguments.arguments.len()), span.line);
            return;
        }
