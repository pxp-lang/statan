...
impl Rule for ValidThisCallRule {
    ...
    fn run(
        ...
        if !context.is_in_class() {
            messages.error(
                format!("Calling $this->{method_name}() outside of class context"),
        ...
        let has_call_magic = class_definition
            .get_method(call_magic, definitions, context)
            .is_some()
            || class_definition
                .get_inherited_method(call_magic, definitions, context)
                .is_some();

        // 7. Check that the method exists.
        if method_definition.is_none() {
            if let Some((inherited_method_from, inherited_method)) =
                class_definition.get_inherited_method(method_name, definitions, context)
            {
                ...
                messages.error(
                    format!(
                        "Call to undefined method $this->{method_name}() on {classish_context}"
                    ),
                    method_call_expression.arrow.line,
                );
                return;
        ...
        if !method.is_public() {
            ...
            if class_definition != method_class_context && method.is_private() {
                messages.error(
                    format!("Call to private method $this->{method_name}()"),
                    method_call_expression.arrow.line,
                );
                return;
            }
        ...
        if method_call_expression.arguments.arguments.len() < min_arity {
            messages.error(
                format!(
                    "Method $this->{}() requires {} arguments, {} given",
                    method_name,
                    min_arity,
                    method_call_expression.arguments.arguments.len()
                ),
                span.line,
            );
            return;
